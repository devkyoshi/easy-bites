...
public class AuthServiceImpl implements AuthService {
    ...
    @Override
    public ApiResponse<UserResponse> registerUser(UpdateRegisterUserRequest updateRegisterUserRequest) {
        if(userRepository.existsByUsername(updateRegisterUserRequest.getUsername())){
            log.error("Username {} already exists", updateRegisterUserRequest.getUsername());
            throw new UserAlreadyExistsException( updateRegisterUserRequest.getUsername());
        }

        //create user
        User user = userFactory.createUser(updateRegisterUserRequest);
    ...
    public ApiResponse<LoginResponse> login(LoginRequest loginRequest) {
        User user = userRepository.findByUsername(loginRequest.getUsername())
                .orElseThrow(() -> new UserNotFoundException(MessageConstants.INVALID_CREDENTIALS));


        if(!passwordEncoder.matches(loginRequest.getPassword(), user.getPassword())){
            log.error("Invalid password for user: {}", user.getUsername());
            throw new UserNotFoundException(MessageConstants.INVALID_CREDENTIALS);
        }
